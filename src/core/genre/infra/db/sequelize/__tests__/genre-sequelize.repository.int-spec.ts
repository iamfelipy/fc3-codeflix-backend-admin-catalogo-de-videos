...
describe('GenreSequelizeRepository Integration Tests', () => {
  ...
  it('should update a entity', async () => {
    ...
    expect(genre.toJSON()).toStrictEqual(entityFound!.toJSON());
    // GenreCategoryModel.count() conta registros na tabela de relação de gênero-categoria direto no banco, não usa o genreRepo. É útil para checar o estado da associação após o update.
    ...
    expect(genre.toJSON()).toStrictEqual({
      ...entityFound!.toJSON(),
      // pq colocar categories_id de novo, se ele já esta em entityFound?
      // Para garantir que o teste não falhe caso a ordem dos categories_id varie, já que o banco pode retornar em ordem diferente.
      categories_id: expect.arrayContaining([
        categories[0].category_id.id,
        categories[1].category_id.id,
      ]),
  ...
  describe('transaction mode', () => {
    ...
    describe('findById method', () => {
      it('should return a genre', async () => {
        ...
        await uow.start();
        await genreRepo.insert(genre);
        // vai passar porque o repositório está usando a mesma transação (uow) para inserir e buscar o gênero, então enquanto a transação está aberta, as operações são visíveis dentro dela, mesmo sem o commit.
        const result = await genreRepo.findById(genre.genre_id);
