  ...
  test('create with association in single transaction ', async () => {
    ...
    const genreWithCategories = await GenreModel.findByPk(genreModel.genre_id, {
      include: [
        {
          model: CategoryModel,
          // O atributo attributes serve para especificar quais colunas (neste caso, apenas category_id) devem ser retornadas do banco de dados ao buscar os registros, otimizando a consulta e retornando apenas o necessário.
          attributes: ['category_id'],
        },
      ],
    });
