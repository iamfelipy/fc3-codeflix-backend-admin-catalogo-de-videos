...
export class GenreSequelizeRepository implements IGenreRepository {
  ...
  async search(props: GenreSearchParams): Promise<GenreSearchResult> {
    ...
    if (props.filter && (props.filter.name || props.filter.categories_id)) {
      ...
      if (props.filter.categories_id) {
        wheres.push({
          ...
          get condition() {
            return {
              // O $ indica um caminho para um campo aninhado numa associação/join no Sequelize. 
    ...
    // Isso retorna o total de gêneros conforme os filtros aplicados.
    const count: number = await this.genreModel.count({
      //  O distinct: true garante que registros duplicados (por join com categorias) não sejam contados duas vezes. 
      distinct: true,
    ...
    /*
      SELECT DISTINCT GenreModel.`genre_id`, <coluna_para_order>
      FROM genres as GenreModel
      [INNER JOIN category_genre ON GenreModel.`genre_id` = category_genre.`genre_id`] -- só se houver filtro por categories_id
      [WHERE GenreModel.name LIKE :name AND category_genre.category_id IN (:categories_id)] -- conforme filtros
      ORDER BY <orderBy>
      LIMIT <limit>
      OFFSET <offset>
    */
