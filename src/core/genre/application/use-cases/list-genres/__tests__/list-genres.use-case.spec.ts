...
describe('ListGenresUseCase Unit Tests', () => {
  ...
  describe('should search applying filter by name, sort and paginate', () => {
    ...
    // O reset ocorre porque novos repositórios (genreRepo, categoryRepo) são instanciados no beforeEach do describe principal, limpando os dados a cada teste.
    // Sim, ambos os beforeEach (do describe principal e do describe interno) são executados antes de cada caso do test.each.
    beforeEach(async () => {
      await categoryRepo.bulkInsert(categories);
      await genreRepo.bulkInsert(genres);
    });
