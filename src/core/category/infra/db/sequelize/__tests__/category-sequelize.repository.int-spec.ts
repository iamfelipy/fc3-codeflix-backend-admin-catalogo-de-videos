  ...
  describe("search method tests", () => {
    it("should only apply paginate when other params are null", async () => {
      ...
      - Pergunta:
          ...
          CategoryModelMapper é usado dentro do repository em outro arquivo, isso vai funcionar pq?
          Resposta:
            Sim, vai funcionar porque CategoryModelMapper é importado como módulo único em todo o projeto. Ao fazer um spy no método estático com Jest, todas as chamadas (inclusive feitas por outros arquivos como o repository) passarão pelo spy, pois usam a mesma instância do módulo em cache pelo Node.js.
      */
      const spyToEntity = jest.spyOn(CategoryModelMapper, "toEntity");
      ...
      // O valor é 15 porque o método search faz paginação (per_page padrão é 15). Assim, apenas 15 das 16 categorias são retornadas na primeira página, chamando toEntity 15 vezes.
    ...
    it("should apply paginate and filter", async () => {
      ...
      expect(searchOutput.toJSON(true)).toMatchObject(
        new CategorySearchResult({
          /*
            Os itens [categories[0], categories[2]] aparecem porque o filtro "TEST" (case-insensitive no SQLite) retorna as categorias cujo nome contém "TEST" ("test", "TEST", "TeSt"), e a ordenação padrão é por created_at decrescente. Assim, os dois primeiros das três filtradas por paginação são [categories[0], categories[2]], pois categories[0] tem o maior created_at entre as filtradas, seguido por categories[2].
          */
    ...
    describe("should search using filter, sort and paginate", () => {
      ...
      // $ = consigo mostrar o valor de uma key do objeto
      // vs %j = mostra o objeto inteiro stringfy
      test.each(arrange)(
