...
describe('CastMemberModelMapper Integration Tests', () => {
  ...
  it('should convert a cast member model to a cast member entity', () => {
    ...
    // O método build do Sequelize cria uma instância do modelo (CastMemberModel), mas não salva no banco de dados. Ele apenas instancia o objeto em memória usando os dados fornecidos. 
    const model = CastMemberModel.build({
