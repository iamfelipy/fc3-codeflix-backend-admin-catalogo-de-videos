  ...
  implements IUseCase<CreateVideoInput, CreateVideoOutput>
{
  ...
  async execute(input: CreateVideoInput): Promise<CreateVideoOutput> {
    ...
    // a cada if você sobrescreve o erro anterior daquele campo, pois o método setError substitui o valor antigo no Map pela nova lista de erros para a mesma chave (field). Se quiser acumular erros, use addError em vez de setError.
