...
export class VideoSequelizeRepository implements IVideoRepository {
  ...
  // binary torna a ordenação case sensitive,
  orderBy = {
    mysql: {
  ...
  async search(props: VideoSearchParams): Promise<VideoSearchResult> {
    // step: configurar paginação
    const offset = (props.page - 1) * props.per_page;
    const limit = props.per_page;

    // step: criar preparativos para query crua
    const videoTableName = this.videoModel.getTableName();
    const videoAlias = this.videoModel.name;
    
    // step: pegar nome das tabelas pivos categories, genres, castmembers para usar na query crua
    const videoCategoryRelation =
      this.videoModel.associations.categories_id.target;
    ...
    // step: construir where com filters não vazios
    const wheres: any[] = [];
    ...
    if (
      ...
      if (props.filter.title) {
        wheres.push({
          ...
          // os conditions vao ser usados no count query
          get condition() {
            return {
              [this.field]: {
                [Op.like]: this.value,
    ...
    // step: montar ordenação
    const orderBy =
      props.sort && this.sortableFields.includes(props.sort)
        ? this.formatSort(props.sort, props.sort_dir)
        : `${videoAlias}.\`created_at\` DESC`;

    // step: descobrir o total para ter metadados de paginação para a camada de apresentação

    /*
      A consulta Sequelize fornecida gerará um SQL semelhante a:

      SELECT COUNT(DISTINCT `video`.`video_id`) AS `count`
      FROM `videos` AS `video`
      INNER JOIN `video_categories` AS `categories_id` ON `video`.`video_id` = `categories_id`.`video_id`
      INNER JOIN `video_genres` AS `genres_id` ON `video`.`video_id` = `genres_id`.`video_id`
      INNER JOIN `video_cast_members` AS `cast_members_id` ON `video`.`video_id` = `cast_members_id`.`video_id`
      WHERE `categories_id`.`category_id` IN (:categories_id)
        AND `genres_id`.`genre_id` IN (:genres_id)
        AND `cast_members_id`.`cast_member_id` IN (:cast_members_id)
        AND `video`.`title` LIKE `%:title%`

      WHERE [condições geradas dinamicamente com base em `wheres.map((w) => w.condition)`]
      Os JOINs só serão incluídos se os filtros correspondentes existirem em props.filter.
      O WHERE será um array de condições combinadas com AND.
      O COUNT é distinto pelo ID do vídeo.
      A transação será usada se fornecida.
    */
    ...
    // step: buscar ids dos videos que atendem aos criterios e com query crua e dinamica
    ...
    // step: buscar videos filtrados com as relações
