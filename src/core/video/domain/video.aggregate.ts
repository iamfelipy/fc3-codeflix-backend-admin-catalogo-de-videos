...
export class Video extends AggregateRoot {
  ...
  // classificação do filme, normativa global
  // : livre, infantil, 12 anos, ...
  rating: Rating;
  // é estreia? se sim, vai aparecer em uma pagina destacada no front-end
  is_opened: boolean; 
  // video é subido cru, depois microserviço converte, e então ele é publicado e pode ser acessado pelas pessoas
  ...
  // ex: documentario, filme, infantil
  categories_id: Map<string, CategoryId>;
  // ex: ação, comedia, terror, suspense, drama
  genres_id: Map<string, GenreId>;
  cast_members_id: Map<string, CastMemberId>;
  ...
  static create(props: VideoCreateCommand) {
    const video = new Video({
      ...
      // regra de negocio valiosa
      // evita que ele seja acessivel no catalogo
      is_published: false,
    });
